...
class ReportGeneralLedgerExcel(models.Model):
    _name = "report.true_mart_pos.report_generalledger_excel"
    _inherit = 'report.report_xlsx.abstract'
    
    def generate_xlsx_report(self, workbook, data, obj):
        report_obj = self.env['report.account.report_generalledger']
...
#                                        'bottom': True, 'top': True, 'bold': True, 'bg_color': '#bfbfbf'})
        ...
        format5 = workbook.add_format({'font_size': 10, 'align': 'right', 'bold': True, 'right': True, 'left': True,
                                       'bottom': True, 'top': True,'bg_color': '#f3f990'})
        ...
        format9 = workbook.add_format({'font_size': 10, 'align': 'left', 'bold': True, 'right': True, 'left': True,
                                       'bottom': True, 'top': True,'bg_color': '#f3f990'})
        ...
        for account in results['Accounts']:            
            for line in account['move_lines']:
                ...
                row += 1
            col = 0
            sheet.merge_range(row, col, row, col + 5, account['code'] + account['name'], format9)
