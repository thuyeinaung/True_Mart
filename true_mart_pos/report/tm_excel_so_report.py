...
class DmsStockReportXls(models.AbstractModel):
    _name = 'report.true_mart_pos.tm_so_report_xls.xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def get_so_data(self, data):
        ...
        self._cr.execute(
                         '''Select am.date Date,rp.name Customer_Name,ai.number Invoice_Number,so.name SO_Number,ap.amount Amount, aj.name Type_of_Payment from sale_order so
                            inner join res_partner rp
                            on rp.id = so.partner_id
                            inner join account_invoice ai
                            on so.name = ai.origin
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        so_query_obj = self.get_so_data(obj)     
        

        sheet = workbook.add_worksheet('SO Report')
        ...
        sheet.merge_range('A1:I2', 'Sales Order Report', format0)
      

        sheet.write(4, 0, 'No.', format21)
        sheet.write(4, 1, 'Date', format21)
        sheet.write(4, 2, 'Customer Name', format21)
        sheet.write(4, 3, 'Invoice Number', format21)
        sheet.write(4, 4, 'SO Number', format21)
        ...
        for each in so_query_obj:   
            ...
            sheet.write(prod_row, prod_col + 2, each['customer_name'], font_size_8)
            sheet.write(prod_row, prod_col + 3, each['invoice_number'], font_size_8)
            sheet.write(prod_row, prod_col + 4, each['so_number'], font_size_8_l) 
            ...
            prod_row = prod_row + 1
            prod_no = prod_no + 1# -*- coding: utf-8 -*-
import datetime
from datetime import datetime
import pytz
from odoo import models, fields
